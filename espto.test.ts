...
describe("test", () => {
    ...
    test("No FedNow or PromoLedger", async () => {
        ...
        // Make sure paymentStatus and datePaid are correct
        testParameterDouble(paymentRequest.parameters!, 'paymentStatus', 4);
        testParameterString(paymentRequest.parameters!, 'datePaid', 
            new Date().toISOString().slice(0, 19).replace('T', ' '),
            false
 ...
 */
function testSingleStatementRequest(
    ...
    testParameterDouble(request.parameters!, 'paymentStatus',
        userPurchaseInformation.paymentMethod !== 0 || userPurchaseInformation.amount === 0 ? 4 : 5);
